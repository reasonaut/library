...
class Library {
  ...
  removeBookFromLibrary(bookIndex) {
    ...
    this.libraryContainer
      .querySelector(`span[data-id="${bookIndex}"]`)
      .remove();
  }
  displayLibrary() {
...
function createBookElement(book) {
  ...
  const readButton = readStatusContainer.appendChild(
...
function toggleReadStatus(eventData) {
  // check current state
  const bookIndex =
    eventData.target.parentElement.parentElement.getAttribute("data-id");
...
function popupWindow(windowName, width, height) {
  const y =
    window.self.top.outerHeight / 2 + window.self.top.screenY - height / 2;
  const x =
    window.self.top.outerWidth / 2 + window.self.top.screenX - width / 2;
  return window.self.open(
    "",
    windowName,
...
function createNewBookForm() {
  ...
  const createNewBookForm = formSpan.appendChild(
    document.createElement("form")
  );
  // title
  const titleInputLabel = createNewBookForm.appendChild(
    document.createElement("label")
  );
  createNewBookForm.appendChild(document.createElement("br"));
  titleInputLabel.setAttribute("for", "titleField");
  titleInputLabel.textContent = "Title";
  const nameInput = createNewBookForm.appendChild(
  ...
  // author
  const authorInputLabel = createNewBookForm.appendChild(
    document.createElement("label")
  );
  createNewBookForm.appendChild(document.createElement("br"));
  authorInputLabel.setAttribute("for", "author");
  authorInputLabel.textContent = "Author";
  const authorInput = createNewBookForm.appendChild(
    document.createElement("input")
  );
  ...
  // pages
  createNewBookForm.appendChild(document.createElement("br"));
  const pagesInputLabel = createNewBookForm.appendChild(
    document.createElement("label")
  );
  createNewBookForm.appendChild(document.createElement("br"));
  pagesInputLabel.setAttribute("for", "pages");
  pagesInputLabel.textContent = "Pages";
  const pagesInput = createNewBookForm.appendChild(
    document.createElement("input")
  );
  pagesInput.id = "pages";
  ...
  // button span
  const buttonSpan = createNewBookForm.appendChild(
    document.createElement("span")
  );
  buttonSpan.id = "formButtonsContainer";
  // confirm button
  const confirmButton = buttonSpan.appendChild(
    document.createElement("button")
  );
...
function createNewBook(eventData) {
  ...
  const isRead = document.getElementById("isReadYes");
  if (
    !title.checkValidity() ||
    !author.checkValidity() ||
    !pages.checkValidity() ||
    !isRead.checkValidity()
  ) {
    if (isNaN(parseInt(pages.value)))
        alert('Pages must be a number!')
    else if (!isRead.checkValidity())
        alert ('Select if this book has been read!')
    else alert('All text fields must be populated!')
    return;
    }
